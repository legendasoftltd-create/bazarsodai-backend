    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>{{translate('messages.flash_sale_setup')}}</span>
            </h1>
        </div>
        ...
        <div class="row g-3">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form action="{{route('vendor.flash-sale.store')}}" method="post">
                            @csrf
                            @if ($language)
                                    ...
                                    <div class="col-12">
                                        <div class="lang_form" id="default-form">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="title[]" id="default_title" class="form-control" maxlength="100" placeholder="{{ translate('messages.ex_:_new_flash_sale') }}">
                                            </div>
                                        ...
                                        @foreach (json_decode($language) as $lang)
                                            <div class="d-none lang_form" id="{{ $lang }}-form">
                                                <div class="form-group">
                                                    ...
                                                    <input type="text" maxlength="100" name="title[]" id="{{ $lang }}_title" class="form-control" placeholder="{{ translate('messages.ex_:_new_flash_sale') }}">
                                                </div>
                        ...
                        <table class="table table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                            ...
                            <tbody id="set-rows">
                                @foreach($flash_sales as $key=>$flash_sale)
                                    <tr>
                                        <td class="text-center">{{$key+$flash_sales->firstItem()}}</td>
                                        ...
                                        <td class="text-center">
                                            <span class="bg-gradient-light text-dark">
                                                {{$flash_sale->start_date ? $flash_sale->start_date->format('d/M/Y').' - '.$flash_sale->end_date->format('d/M/Y') : 'N/A'}}
                                            </span>
                                        </td>
                                        <td class="text-center">{{ $flash_sale->activeProducts->count() }}</td>
                                        <td class="text-center">
                                            ...
                                            <form action="{{route('vendor.flash-sale.publish',[$flash_sale['id'],$flash_sale->is_publish?0:1])}}" method="get" id="is_publish-{{$flash_sale['id']}}_form"></form>
                                        </td>
                                        <td>
                                            <div class="btn--container justify-content-center">
                                                <a class="btn p-2 btn--primary btn-outline-primary" href="{{route('vendor.flash-sale.add-product',[$flash_sale['id']])}}" title="{{translate('messages.add-product')}}">
                                                    <i class="tio-add"></i>{{ translate('messages.Add_new_product') }}
                                                </a>
                                                <a class="btn action-btn btn--primary btn-outline-primary" href="{{route('vendor.flash-sale.edit',[$flash_sale['id']])}}" title="{{translate('messages.edit')}}">
                                                    <i class="tio-edit"></i>
                                                </a>
                                                <a class="btn action-btn btn--danger btn-outline-danger form-alert" href="javascript:"
                                                    data-id="flash_sale-{{$flash_sale['id']}}"
                                                    data-message="{{ translate('Want to delete this flash_sale ?') }}"
                                                    title="{{translate('messages.delete')}}">
                                                    <i class="tio-delete-outlined"></i>
                                                </a>
                                                <form action="{{route('vendor.flash-sale.delete',[$flash_sale['id']])}}" method="post" id="flash_sale-{{$flash_sale['id']}}">
                                                    @csrf @method('delete')
                                                </form>
                                            </div>
                                        </td>
