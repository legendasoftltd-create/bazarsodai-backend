...
@section('title', translate('Store Statement'))
        ...
        <div class="row align-items-center">
            <div class="col-sm mb-2 mb-sm-0">
                <h1 class="page-header-title">
                    <span class="page-header-icon"><i class="tio-filter-list nav-icon"></i></span>
                    {{ translate('Store Statement') }}
                    <small class="text-muted font-weight-normal ml-2">{{ translate('Account 2011 — Store Wallet Payable') }}</small>
                </h1>
            </div>
            @if($statement)
    ...
    <div class="card mb-3">
        <div class="card-body py-2">
            <form method="GET" class="row g-2 align-items-end">
                <div class="col-md-4">
                    <label class="form-label mb-1">{{ translate('Store') }}</label>
                ...
                <div class="col-md-2">
                    <a href="{{ route('admin.accounts.reports.store-statement') }}" class="btn btn-outline-secondary btn-block">{{ translate('Reset') }}</a>
                </div>
            </form>
        </div>
    </div>
        ...
        <div class="row mb-3">
            ...
            <div class="col-md-4">
                <div class="card card-body text-center border-left border-primary">
                    ...
                    <small class="text-muted">{{ translate('Amount owed to store') }}</small>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card card-body text-center">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center py-4 text-muted">
                                    {{ translate('No entries for this store in the selected period.') }}
                                </td>
                            </tr>
                        @endforelse
    ...
    @else
        <div class="card">
            <div class="card-body text-center py-5 text-muted">
                <i class="tio-filter-list" style="font-size:3rem"></i>
                <p class="mt-2">{{ translate('Select a store above to view its statement.') }}</p>
            </div>
        </div>
    @endif
