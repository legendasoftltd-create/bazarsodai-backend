    ...
    <div class="card mb-3">
        <div class="card-body py-2">
            <form method="GET" class="row g-2 align-items-end">
                <div class="col-md-3">
                    <input type="date" name="from" class="form-control" value="{{ request('from') }}" placeholder="{{ translate('From') }}">
                </div>
                <div class="col-md-3">
                    <input type="date" name="to" class="form-control" value="{{ request('to') }}" placeholder="{{ translate('To') }}">
                </div>
                <div class="col-md-2"><button class="btn btn-primary btn-block">{{ translate('Filter') }}</button></div>
                <div class="col-md-2">
                    <a href="{{ route('vendor.inventory.reports.purchases') }}" class="btn btn-outline-secondary btn-block">{{ translate('Reset') }}</a>
                </div>
                <div class="col-md-2 text-right">
                    <a href="{{ route('vendor.inventory.reports.export', 'purchases') }}?{{ http_build_query(request()->all()) }}" class="btn btn-outline-success btn-block">
            ...
            <div class="table-responsive">
                <table class="table table-hover table-borderless table-thead-bordered table-align-middle card-table">
                    ...
                    <tbody>
                        ...
                        @php $isReturn = $m->type === 'purchase_return'; @endphp
                        <tr>
                            ...
                            <td class="{{ $isReturn ? 'text-danger' : 'text-success' }}">{{ $m->qty }}</td>
                        ...
                        @empty
                        <tr><td colspan="7" class="text-center py-4">{{ translate('No purchase records found') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
